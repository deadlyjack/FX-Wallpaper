...
public class Wallpaper extends CordovaPlugin {
  ...
  public boolean execute(String action, final JSONArray args, final CallbackContext callbackContext)
    ...
    if (action.equals("set-wallpaper")) {
      this.setWallpaper(callbackContext, args.getString(0), args.getString(1), args.getDouble(2), args.getDouble(3),
  ...
  private void getWallpaper(final CallbackContext callbackContext, final String which) {
    int wallpaperof;
    ...
    cordova.getThreadPool().execute(new Runnable() {
      public void run() {
        try {
          ...
          if (API >= 24) {
            ParcelFileDescriptor wallpaperFile = wallpaperManager.getWallpaperFile(wallpaperof);
          ...
          File outputFile = File.createTempFile(".wallpaper", ".jpg", context.getCacheDir());
  ...
  private void setWallpaper(final CallbackContext callbackContext, final String which, final String fileUri,
    ...
    cordova.getThreadPool().execute(new Runnable() {
      public void run() {
        ...
        if (which.equals("LOCK")) {
          ...
          setFor = WallpaperManager.FLAG_LOCK | WallpaperManager.FLAG_SYSTEM;
        }

        try {

          if (API >= 28) {
            ImageDecoder.Source imgSrc = ImageDecoder.createSource(context.getContentResolver(), uri);
          ...
          if (API >= 24) {
            ...
            bitmap = Bitmap.createBitmap(bitmap, left, top, desiredWidth, desiredHeight);
  ...
  private void compress(final CallbackContext callbackContext, final String src, final int quality) {
    final Context context = this.context;
    final WallpaperManager wallpaperManager = this.wallpaperManager;
